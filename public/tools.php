 ...
 * @author                          Minko Alexander
 */
function executeRest($method, $params = array(), $domain, $accessToken, $userId)
{
    ...
    if (array_key_exists('error', $result)) {
        unlink(__DIR__ . '/reports/flag.txt');
        sleep(1);
        $curl = curl_init("https://" . $domain . "/rest/im.notify.json");
        curl_setopt_array($curl, array(
                CURLOPT_POST            => 1,
                CURLOPT_RETURNTRANSFER  => 1,
                CURLOPT_POSTFIELDS      => http_build_query(array_merge(array('to' => $userId, 'message' => $result['error'] . ': ' . $result['error_description'] . '.', 'type' => 'SYSTEM'), array("auth" => $accessToken))))
        );
        curl_exec($curl);
        curl_close($curl);
        exit();
...
function getInfo ($arFilter = array(), $beginDate, $endDate, $domain, $token, $lang, $userId)
{
    ...
    if (count($arDeals['result']) > 0) {
        ...
        sleep(1);
        $arUser = executeRest('user.get', array('ID' => $arDeals['result'][0]['CREATED_BY_ID']), $domain, $token, $userId);
        $arDeal['CREATOR'] = '';
        ...
        $arUser = executeRest('user.get', array('ID' => $arDeals['result'][0]['ASSIGNED_BY_ID']), $domain, $token, $userId);
        $arDeal['RESPONSIBLE'] = '';
...
function getDeal ($arProject, $beginDate, $endDate, $domain, $token, $refreshId, $userId)
{
            ...
            if (count($arTasks['result']) > 0) {
                ...
                foreach ($arTasks['result'] as $arTask) {
                    ...
                    do {
                        ...
                        if (floor($duration/60) > 50) {
                            $curl = curl_init("https://oauth.bitrix.info/oauth/token/?grant_type=refresh_token&client_id=local.5c98f2ed6d3c52.41098832&client_secret=T2XnGnwxYckvcvcpTW74FZ5xKGRK3YzrDnfwXQ5C4ct3Y267Ju&refresh_token=" . $refreshId);
                            curl_setopt_array($curl, array(
                                    CURLOPT_POST            => 1,
                                    CURLOPT_RETURNTRANSFER  => 1,
                            ...
                            $token      = $arResult['access_token'];
                            $refreshId  = $arResult['refresh_token'];
                            $beginTime  = microtime(true);
                        }
